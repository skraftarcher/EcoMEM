...
# look at species richness, diversity, and evenness w the change in percent cover 
...
# analyze species richness and change in percent cover----------
par(mfrow=c(2,2))
spr<-lm(spr~scar*pchangepc*sampling,data=comd)

plot(spr)
summary(spr)
anova(spr)

# analyze diversity and change in percent cover----------
par(mfrow=c(2,2))
div<-lm(div~scar*pchangepc*sampling,data=comd)

plot(div)
summary(div)
anova(div)

# change in scallop abundance with change in percent cover----------
scallops<-aov(sqrt(pchange+0.01)~scar*pchangepc*sampling,data=comd)

par(mfrow=c(2,2))
plot(scallops)
summary(scallops)

# analyze species evenness and change in percent cover----------
...
# and get pchangepc data from visual survey analysis
dip.comd <- dip.com2%>%
  ungroup()%>%
  left_join(comd[, c("plotID", "sampling", "time.var", "pchangepc")],
            by = c("plotID", "sampling", "time.var"))

# analyze species richness and change in percent cover----------

par(mfrow=c(2,2))
spr<-lm(sqrt(spr)~scar*pchangepc,data=dip.comd%>%
...
# analyze diversity and change in percent cover----------
par(mfrow=c(2,2))
div<-lm(div~scar*pchangepc,data=dip.comd%>%
          filter(taxa == "Acteocina canaliculata", sampling == "s2"))
...
lp("DHARMa")
lp("glmmTMB")
# change in mud crab abundance with change in percent cover----------
deca2<-glmmTMB(abund~scar*pchangepc,data=dip.comd%>%
...
# change in lunar dove snail abundance with change in percent cover----------
gast20<-glmmTMB(abund~scar*pchangepc,data=dip.comd%>%
...
# change in gast 4 abundance with change in percent cover----------
gast4<-glmmTMB(abund~scar*pchangepc,data=dip.comd%>%
...
# change in common eastern nassau abundance with change in percent cover----------
gast15<-glmmTMB(abund~scar*pchangepc,data=dip.comd%>%
