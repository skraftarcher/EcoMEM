...
par(mfrow=c(2,2))
spr<-lm(spr~graze+pchangesd+sampling,data=comd)
...
par(mfrow=c(2,2))
div<-lm(div~graze+pchangesd+sampling,data=comd)
...
scallops<-aov(pchange~graze*pchangesd*sampling,data=comd)
...
par(mfrow=c(2,2))
spr<-lm(spr~graze*pchangesd,data=dip.comd%>%
...
# analyze diversity and change in shoot density----------
par(mfrow=c(2,2))
div<-lm(div~graze*pchangesd,data=dip.comd%>%
          filter(taxa == "Acteocina canaliculata", sampling == "s2"))
...
deca2<-glmmTMB(abund~graze*pchangesd,data=dip.comd%>%
...
gast20<-glmmTMB(abund~graze*pchangesd,data=dip.comd%>%
...
gast4<-glmmTMB(abund~graze*pchangesd,data=dip.comd%>%
...
gast15<-glmmTMB(abund~graze*pchangesd,data=dip.comd%>%
...
ggplot(data=comd,aes(x=pchangesd, y=pchange, color=graze, fill=graze))+
  geom_jitter(alpha = 0.5, width = 0.2)+
  geom_point()+
  ...
  scale_color_manual(
    ...
    values = c("no graze" = "#94D2BD", "graze" = "#005F73"),
    labels = c("No Graze", "Grazed")
  ) +
  scale_fill_manual(
    name = "Seagrass Treatment",
    values = c("no graze" = "#94D2BD", "graze" = "#005F73"),
  ...
  labs(
    title = "Interactive Effects of Seagrass Density and Grazing over Time on Scallop Abundance Change",
